...
def follow_users(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save() # Modelform.save() passe du form au model
...
def home(request):
    ...
    # --3--filtrer et trié par date
    ## trié que blog
    ## blogs = blogs.order_by('-date_created')
    ## trié les 2 en même temps
    blogs = models.Blog.objects.filter(
        Q(author__in=request.user.follows.all()) | Q(starred=True))
    photos = models.Photo.objects.filter(
    ...
    context = {'blogs_and_photos': page_obj}
